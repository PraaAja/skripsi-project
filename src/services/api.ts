...
export const recommendationAPI = {
  ...
  async processRecommendation(scores: any, userId: number) {
    const res = await fetch("http://localhost:5000/api/siswa/process", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
export const guruAPI = {
  async getDashboardStats() {
    const response = await fetch("http://localhost:5000/api/guru/dashboard");
  ...
  const response = await fetch("http://localhost:5000/api/guru/siswa");
  const data = await response.json();
  if (!response.ok) throw new Error(data.message);
  return data;
},
...
async updateStatusSiswa(id: number) {
  const res = await fetch(
    `http://localhost:5000/api/guru/siswa/${id}/proses`, // 🔥 FIX DI SINI
    {
      method: "PUT",
    }
  );
  ...
  async getRekomendasi() {
    const response = await fetch("http://localhost:5000/api/guru/rekomendasi");
    const data = await response.json();
    if (!response.ok) throw new Error(data.message);
    return data;
  }
